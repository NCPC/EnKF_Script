...
FREE_CASENAME='NorCPM_F19_tn21_mem'
...
for yr=year_start:year_end
   ...
   for month=1:12
      ...
      for mem=1:NENS
         %reading Free
         f_sst=ncgetvar([FREE_PATH  FREE_CASENAME  num2str(mem,'%2.2d') '.micom.hm.' num2str(yr,'%4.4d') '-' num2str(month,'%2.2d') '.nc'],'sst');
         avg_sst=ncgetvar([Input_PATH 'ave-' num2str(month,'%2.2d') '.nc'],'sst');
         f_sst=f_sst-avg_sst;
         f_sic=ncgetvar([FREE_PATH   FREE_CASENAME num2str(mem,'%2.2d') '.micom.hm.' num2str(yr,'%4.4d') '-' num2str(month,'%2.2d') '.nc'],'fice');
         mask=find(f_sic~=0);
         f_sst(mask)=nan;
         %reading Assim
         a_sst=ncgetvar([NorCPM_PATH   CASENAME num2str(mem,'%2.2d') '.micom.hm.' num2str(yr,'%4.4d') '-' num2str(month,'%2.2d') '.nc'],'sst');
         a_sst=a_sst-avg_sst;
         a_sic=ncgetvar([NorCPM_PATH   CASENAME num2str(mem,'%2.2d') '.micom.hm.' num2str(yr,'%4.4d') '-' num2str(month,'%2.2d') '.nc'],'fice');
         mask=find(a_sic~=0);
         a_sst(mask)=nan;
         %interp_model to obs
         for i=1:dlon
...
whitebg('w');
hold on
m_proj('Miller','lon',[-80 30],'lat',[-30 30]);
...
m_grid;
m_coast('patch','k');
print('-depsc2',['Mean_Monthly_std_FREE_' CASENAME 'SSTA.eps'])
figure(2)
set(gcf, 'Renderer', 'opengl')
set(gcf, 'InvertHardCopy', 'off');
whitebg('w');
...
m_coast('patch','k');
colorbar('h')
print('-depsc2',['Mean_Monthly_std_NorCPM_' CASENAME 'SSTA.eps'])
%
figure(3)
m_proj('Miller','lon',[-80 30],'lat',[-30 30]);
set(gcf, 'Renderer', 'opengl')
...
m_coast('patch','k');
colorbar('h')
print('-depsc2',['Mean_Monthly_std_obs_SSTA' CASENAME '.eps'])
