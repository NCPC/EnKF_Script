...
do
 ...
 do
    ...
    if (( ${SKIPASSIM} ))
       ...
       do
          ...
          then
             ...
             if (( ${SUPERLAYER} ))
                then
                   cp -f ${HOMEDIR}/bin/EnKF_Yiguo_anom_no_copy EnKF
                else
          ...
          ln -sf  $GRIDPATH .
          ${HOMEDIR}/Script/Link_forecast_nocopy.sh ${yr} ${month}
          ./prep_obs 
          ...
          cat ${WORKSHARED}/Input/EnKF/fixenkf_${RES}.sh_480_mal | sed  "s/NENS/${ENSSIZE}/g"  > fixenkf.sh
          chmod 755 fixenkf.sh
          cp -f ${HOMEDIR}/bin/micom_serial_init_${RES}-16-nocopy micom_serial_init
          cp -f ${HOMEDIR}/bin/launcher${ENSSIZE} launcher
          ${HOMEDIR}/Script/Post_process_parallel_nocopy.sh ${yr} ${month} ${ENSSIZE}
          if [ ! -f "FINITO" ] 
          then
    ...
    if (( ${SKIPPROP} ))
       ...
       cd ${HOMEDIR}/Script/
       jobid=`qsub Integrate_ME_1_month_superjob.sh`
       finished=0
       while (( ! finished ))
       do
          finished=1
